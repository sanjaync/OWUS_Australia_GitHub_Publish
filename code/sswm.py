...
class SM_C_H(object):
    ...
    def __init__(self, params, nbins=100, q=1, f_wilt=0.05, f_star=0.95, constraints=True, stress_type='dynamic'):
        ...
        [self.T_GS, self.rf_alpha, self.rf_lambda, self.Eo, self.Td,
          ...
          self.Ps0, self.b, self.Ks, self.n, s_h_in, s_fc_in, 
          self.k_xl_max, self.Px50, self.Pg50, 
        ]  = params

        # Always recalculate s_h and s_fc from soil potential
        # Upstream difference: Upstream uses params directly (s_h, s_fc) passed in __init__
        # self.s_h = self.pot_to_s_BC(-10) # 10MPa
        if np.isnan(s_h_in):
             self.s_h = self.pot_to_s_BC(-10)
        else:
             self.s_h = s_h_in
        # s_fc logic match upstream (simplified)
        s_fc_ref = self.pot_to_s_BC(-0.033) # 0.033MPa
        if np.isnan(s_fc_in):
            self.s_fc = s_fc_ref
        else:
            self.s_fc = s_fc_in # Upstream trusts the input s_fc
            # self.s_fc = np.max([s_fc_ref, s_fc_in])

        self.phiE = np.exp(-self.k_ext * self.LAI)
        self.delta = self.hi * self.LAI
        
        if self.rf_alpha > self.delta:
            self.rf = (self.rf_alpha - self.delta) * self.rf_lambda
            self.g_p = self.n * self.Zm / (self.rf_alpha - self.delta)
        else:
            self.rf = 0
            self.g_p = 0 # Will be ignored as et_ok will be 0
            self.et_ok = 0
        ...
        if (self.et_ok == 0) or (self.epsilon <=0) or (self.AA > 1) or (self.beta_ww > 1) or \
           np.isnan(self.s_h) or np.isnan(self.s_fc) or np.isnan(self.s_star) or np.isnan(self.s_wilt):
            # if self.et_ok != 0:
            #     print(f"DEBUG: sswm et_ok=0 due to: e={self.epsilon}, AA={self.AA}, bww={self.beta_ww}, sh={self.s_h}, sfc={self.s_fc}")
            self.et_ok = 0
            self.beta_ww = np.nan
            self.s_fc = np.nan
            self.s_star = np.nan
        ...
        if self.et_ok == 1 and np.any(np.isnan(self.p0)):
            print("DEBUG: sswm et_ok=0 due to NaN in p0")
            self.et_ok = 0
    ...
    def __log_p0(self, s):
        ...
        if rho <= 0: # Avoid log(0) or division by zero
            return -np.inf

        if (s > self.s_h) and (s <= self.s_wilt):
            # p = ((np.exp(- self.g_p * s)) * (s - self.s_h) ** self.a) / rho
            log_p = -self.g_p * s + self.a * np.log(s - self.s_h) - np.log(rho)

        elif (s > self.s_wilt) and (s <= self.s_star):
            # p1 = ((np.exp(- self.g_p * s))* (self.s_wilt - self.s_h) ** self.a) / rho
            log_p1 = -self.g_p * s + self.a * np.log(self.s_wilt - self.s_h) - np.log(rho)
            
            # p2 = ((s - self.s_h)  / (self.s_wilt - self.s_h) ... ** self.w
            term_inner = (s - self.s_h) / (self.s_wilt - self.s_h) + \
                         (self.T_ww * (s - self.s_wilt) * (self.s_fc - self.s_h)) / \
                         (self.E_max * (self.s_star - self.s_wilt) * (self.s_wilt - self.s_h))
            log_p2 = self.w * np.log(term_inner)
            ...
            term_inner_p2 = (self.s_star - self.s_h) / (self.s_wilt - self.s_h) + \
                            (self.T_ww * (self.s_fc - self.s_h)) / \
                            (self.E_max * (self.s_wilt - self.s_h))
            log_p2b = self.w * np.log(term_inner_p2)
            
            term_inner_p3 = (self.E_max * (s - self.s_h) + self.T_ww * (self.s_fc - self.s_h)) / \
                            (self.E_max * (self.s_star - self.s_h) + self.T_ww * (self.s_fc - self.s_h))
            log_p3 = self.a * np.log(term_inner_p3)
            
            log_p = log_p1 + log_p2b + log_p3
            ...
            term_inner_p3 = ((self.E_max + self.T_ww) * (self.s_fc - self.s_h)) / \
                            (self.E_max * (self.s_star - self.s_h) + self.T_ww * (self.s_fc - self.s_h))
            log_p3b = self.a * np.log(term_inner_p3)
            ...
            log_p4 = self.l_p * ee / (self.bb * (self.m - self.E_max - self.T_ww))
            
            log_p = log_p1 + log_p2b + log_p3b + log_p4
    ...
    def get_p0(self):
        ...
        log_p0 = np.array([self.__log_p0(s) for s in s_list])
        
        # Normalize: p = exp(log_p - max_log_p)
        max_log_p = np.max(log_p0)
        
        # Handle case where all probabilities are -inf (impossibility)
        if max_log_p == -np.inf:
            return np.zeros_like(log_p0) + 1e-10
            
        p0 = np.exp(log_p0 - max_log_p)
        
        # Normalize sum to match PDF property (sum * ds = 1, but extracting shape here)
        c = np.sum(p0)
        return p0 / c

    def __p0_deprecated(self, s):
        # Original __p0 moved here or deleted. Keeping logic in __log_p0
        pass
    ...
    def eval_et_ok(self):
        ...
        if (self.s_fc <= self.s_h) or (self.s_star <= self.s_wilt) or (self.s_fc <= self.s_wilt):
            return 0
            
        # Check for NaNs in critical thresholds
        if np.isnan([self.s_h, self.s_wilt, self.s_star, self.s_fc]).any():
            return 0
    ...
    def mean_stress_duration(self, p0):
        P0 = np.cumsum(p0)
        l = (len(p0) - 1) 
        P_star = P0[int(np.rint(self.s_star * l))]
        p_star = p0[int(np.rint(self.s_star * l))]
        rho_star = self.loss(self.s_star)
        if (p_star * rho_star) == 0:
            return 1e10 # Large duration if loss is zero
