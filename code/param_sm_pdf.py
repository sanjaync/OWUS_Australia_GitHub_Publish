...
class Inverse_bayesian_fitting(object):
    ...
    def mcmc_mh(self, theta0_dict):
        ...
        if (li, theta_i) != (np.nan, np.nan) and not np.isinf(li):
            result = [None] * (int(self.nbr_sim * (1 - self.burnin)))
            for it in range(self.nbr_sim):
                if it % 1000 == 0:
                    print(f"DEBUG: it {it}/{self.nbr_sim}, accepted={accepted}")
                ...
                if it >= self.nbr_sim * self.burnin:
                    ...
                    result[int(it - self.nbr_sim * self.burnin)] = [l_acc, theta_list]
        ...
        if while_count < maxcount:
            if self.epsi is None:
                ...
                if smpdf.et_ok == 1:
                    ...
                    if np.isnan(smpdf.s_h) or np.isnan(smpdf.s_fc):
                        print(f"CRITICAL: smpdf.et_ok=1 but s_h={smpdf.s_h}")
                        l = np.nan
                else:
                    l = np.nan
            else:
            ...
            while l == - np.inf and while_count < maxcount:
                ...
                if self.epsi is None:
                    smpdf = self.SM_PDF(params, nbins=self.nbins, stress_type=self.stress_type)
                    if smpdf.et_ok == 1:
                        if not np.isnan(smpdf.s_h) and not np.isnan(smpdf.s_fc):
                            l = self.eval_loglikelihood(smpdf.p0, self.s_obs, params)
                        else:
                            l = np.nan
                    else:
            ...
            if while_count < maxcount:
                ...
                print('x failed finding non-inf l after maxcount', params)
                return np.nan, np.nan
        else:
            print('x failed finding consistent model after maxcount', params)
            return np.nan, np.nan
    ...
    def test_model_consistency(self, params):
        ...
        if Px50 > self.p_ranges['Px50'][1] or \
                ...
                lnan > 0:
            # if lnan > 0:
            #    nans = [self.full_param_name_list[i] for i, k in enumerate(params) if np.isnan(k)]
            #    print(f"DEBUG: consistency fail NaN: {nans}")
            # print(f"DEBUG: consistency fail basic: Px50={Px50}, Pg50={Pg50}, k_xl_max={k_xl_max}, RAI={RAI}, lneg={lneg}, lnan={lnan}")
            return 0
        ...
        if pi_R > self.p_ranges['pi_R'][1] or \
                pi_R < self.p_ranges['pi_R'][0] or \
                pi_F > self.p_ranges['pi_F'][1] or \
                pi_F < self.p_ranges['pi_F'][0]:
            # print(f"DEBUG: consistency fail pi: pi_R={pi_R}, pi_F={pi_F}")
            return 0

        x = (pi_F / 2 + 1) ** 2 - 2 * pi_F * pi_R 
        if x < 0:
            # print(f"DEBUG: consistency fail x < 0: x={x}")
            return 0
            
        beta_ww = 1 - 1 / (2 * pi_R) * (1 + pi_F / 2 - x ** 0.5)

        if beta_ww > self.p_ranges['beta_ww'][1] or \
                beta_ww < self.p_ranges['beta_ww'][0]:
            # print(f"DEBUG: consistency fail beta_ww: {beta_ww}")
            return 0
    ...
    def mcmc_mh_criteria(self, li, theta_i):
        ...
        if lii > li:
            # print(f"DEBUG: Accepted (lii > li): {lii} > {li}")
            return [1, lii, theta_ii]
        else:
            prob = np.exp(lii - li)
            u = np.random.uniform(0.0, 1.0)
            if u < prob:
                # print(f"DEBUG: Accepted (prob): {u} < {prob}")
                return [1, lii, theta_ii]
            else:
                return [0, li, theta_i]

    def eval_mh_model(self, theta_i):
        w_in = self.w
        params = self.make_mh_model(theta_i, w=w_in)
        if self.test_model_consistency(params) == 1:
            ...
            # if it_debug is not None and it_debug % 1000 == 0:
            #    print(f"DEBUG: Consistency failed for proposal")
            l = -np.inf
            smpdf = []
        return l, smpdf
...
class Processor(object):
    ...
    def get_mcmc_mh_results(self, s_obs, params_dict0, p_ranges, 
        ...
        while (len(pl_results) < num_pl) and (it_count < max_num_pl):
            ...
            if x[1] != 'max init random':
                ...
                print('it', it_count, len(pl_results), 'max init random', fail_eff_count, num_pl)
    ...
    def process_raw_results(self, result_dict, pl_results, p_ranges, outfile_format='short'):
        ...
        f = interp1d(cdf, np.linspace(0, 1, len(p_fitted_norm)), bounds_error=False, fill_value=(0, 1))
        ...
        s_mod = [(np.abs(cdf_m_n - qi)).argmin() / float(len(p_fitted_norm) - 1) for qi in q_obs]
        result_dict['NSE_O'] = __nse(result_dict['s_obs'], s_mod)

        s_mod_2 = [(np.abs(cdf_m_n - qi / 365.)).argmin() / float(len(p_fitted_norm) - 1) for qi in range(1, 365)]
        ...
        result_dict['stress_type'] = self.stress_type
        return result_dict
