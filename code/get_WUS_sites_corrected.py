...
p_ranges = {'k_xl_max': [0.1 * 10**(-3), 1.5 * 10**(-3)], 
            'Px50': [-5, -0.5], 
            'Pg50': [-4.0, -0.5],
            'RAI': [1, 15], 
            'Zm': [0.05, 1], 
...
sites = pd.read_csv('/home/sanjays/et97_scratch2/oldscratch/Ozflux_data_full/OWUS_australia_agent_2/sanjay data creation/master_files/pedo_master_file.csv')
...
# MH-MCMC fit .................................................................................
    ...
    for criteria in ['bf', 'opt']:
        resultpath_plots =  './output_corrected/%s/figs/%s' % (criteria, fn)
        resultpath_pickles =  './output_corrected/%s/files/%s' % (criteria, fn)
        
        os.makedirs(resultpath_pickles, exist_ok=True)
        os.makedirs(resultpath_plots, exist_ok=True)

    for site in sites['siteID'].values: 
        for criteria, epsi in [['bf', None], ['opt', 1], ]:
            ...
            try:
                ...
                if not os.path.exists(f_params):
                    print(f"Skipping {site}: {f_params} not found")
                    continue
                ...
                try:
                    # CORRECTED: Use 5 parallel chains for proper MCMC convergence
                    # Original num_pl=1 cannot detect convergence failures
                    loc_res = go_mhmcmc(params, unknown_params, save_params, epsi=epsi, stress_type=stress_type, num_pl=5, nbins=100)
                    ...
                    if 'NSE' in loc_res:
                        ...
                        print(f"MCMC finished for {site} but did not converge/produce NSE. Success status: {loc_res.get('num_pl', 0)}")
                except Exception as e:
                    print(f"Failed {site} {criteria}: {e}")
                    import traceback
                    traceback.print_exc()
            except Exception as e:
                print('xxx overall', site, e)
...
# combine results ................................................................................
    ...
    for criteria in ['bf', 'opt']:
        res_dir = './output_corrected/%s/files/%s' % (criteria, fn)
        outfile = './output_corrected/combined/results_%s_%s.csv' % (criteria, fn)
        os.makedirs('./output_corrected/combined', exist_ok=True)
        results_all = []
        for site in sites['siteID'].values:
            print(site, '.................')
            outfile_res = os.path.join(res_dir, '%s.pickle' % site)
            if not os.path.exists(outfile_res):
                print(f"Skipping {site} summary: result not found")
                continue
            with open(outfile_res, 'rb') as f:
                loc_res = load(f)
            for k in list(loc_res.keys()):
            ...
            results_all.append(pd.DataFrame(loc_res, index=[0]))
        ...
        if results_all:
            ...
            print(f"No results found for {criteria}")
